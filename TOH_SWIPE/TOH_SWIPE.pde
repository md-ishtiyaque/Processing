...
void setup(){
  ...
  //text("SELECT THE NUMBER OF DISCS.", width/7, height/4);
    for(int j = 2; j < 10; j+=2){
      text(j, (0.5) * width/2, (j) * height/10 );
      text(j+1, (1.5) * width/2, (j) * height/10 );
    }
  noStroke();
}
...
void mousePressed(){
  ...
  if(!discCountSelect){
    if(mouseX > width/2){
      if(mouseY<height/4){ discCount = 3; }
      else if(mouseY<height/2){ discCount = 5; }
      else if(mouseY<3*height/4){ discCount = 7; }
      else{ discCount = 9; }
    }
    else{
      if(mouseY<height/4){ discCount = 2; }
      else if(mouseY<height/2){ discCount = 4; }
      else if(mouseY<3*height/4){ discCount = 6; }
      else{ discCount = 8; }
    }
...
void mouseReleased(){
  count = 0;
  if(dragged){
    if(selectA){ A[++topA] = temp; }
    if(selectB){ B[++topB] = temp; }
    if(selectC){ C[++topC] = temp; }
  }
  
  if(discCountSelect && dragged){
    //Tower A
    if(mouseX < width / 3){
      selectA = !selectA;
...
void mouseDragged(){
  ...
  dragged = true;
  if(count == 1){
  if(selectA){ temp = A[topA]; A[topA--] = 0; }
  if(selectB){ temp = B[topB]; B[topB--] = 0; }
  if(selectC){ temp = C[topC]; C[topC--] = 0; }
  }
  renderBackground();
  for(int i = 0; i < discCount; i++){
    if(A[i] > 0){
  ...
  fill(map(temp, 1, discCount + 1, 0, 360), 100, 100);
  rect(mouseX, mouseY, map(temp, 1, discCount, width/12, width/4), 50, 15);
}
